...
public class AABoatItem extends Item {
    private static final Predicate<Entity> RIDERS = EntitySelector.NO_SPECTATORS.and(Entity::isPickable);
    private final AABoatEntity.BoatType type;
    private final boolean chest;

    public AABoatItem(boolean chest, AABoatEntity.BoatType type, Item.Properties settings) {
        super(settings);
        this.chest = chest;
        this.type = type;
    }
    ...
    public InteractionResultHolder<ItemStack> use(Level world, Player user, InteractionHand hand) {
        ...
        if (hitResult.getType() == HitResult.Type.BLOCK) {
            AABoatEntity boatEntity = this.createEntity(world, hitResult);
            boatEntity.setAABoatType(this.type);
    ...
    private AABoatEntity createEntity(Level world, HitResult hitResult) {
        if (this.chest) {
            return new AAChestBoatEntity(world, hitResult.getLocation().x, hitResult.getLocation().y, hitResult.getLocation().z);
        }
        return new AABoatEntity(world, hitResult.getLocation().x, hitResult.getLocation().y, hitResult.getLocation().z);
    }
